...
from utils import load_json_time_config
...
def subclip_audio(
    ...
    max_subclip_size_mb: float = 25.0,
) -> dict:
    ...
    if pause_start_time and pause_end_time:
        ...
        pasue_to_end = audio_clip.subclip(
            pause_end_time.total_seconds(), audio_clip.duration
        )
    ...
    if audio_start:
        audio_clip = audio_clip.subclip(
            audio_start.total_seconds(), audio_clip.duration
        )
    ...
    nb_subclips = calculate_number_subclips(
        mp3_path=mp3_path, mb_limitation=max_subclip_size_mb
    )
    single_subclip_duration = ceil(duration / nb_subclips)
    subclips_timestamps = [
        dict(
            index=str(int(i / single_subclip_duration)),
            start=max(i - stride, 0),
            end=min(i + single_subclip_duration + stride, duration),
        )
        for i in range(0, duration, single_subclip_duration)
    ]  # {str(int(i/single_subclip_duration)):(i, i+single_subclip_duration) for i in range(0, duration, single_subclip_duration)}
    for subclip in subclips_timestamps:
        temp_clip = audio_clip.subclip(subclip["start"], subclip["end"])
        subclip[
            "path"
        ] = f"{saving_folder}/subclip_{subclip['index']}_{subclip['start']}_{subclip['end']}.mp3"
...
if __name__ == "__main__":
    ...
    JSON_FILE_PATH = "output/BIOENG320_lecture2(2023)_Prof-Yimon-AYE.json"

    time_config = load_json_time_config(JSON_FILE_PATH, "audio")

    MP4ToMP3(VIDEO_FILE_PATH, AUDIO_FILE_PATH)

    subclips_list = subclip_audio(
        AUDIO_FILE_PATH,
